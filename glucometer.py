...
from glucometerutils.sync_support import sync
...
def main():
  ...
  parser.add_argument(
    '--server', action='store',
    help='Select the path to send data to the server.')

  parser.add_argument(
    '--db', action='store',
    help='Select the path to the personal database log.')
    ...
    if args.action == 'info':
      ...
      for reading in readings:
        if args.db is not None:
          try:
            try:
              db_log.create_db(args.db)
              db_log.dump2db(reading,device_info,time_str,args.db)
            except:
              db_log.dump2db(reading,device_info,time_str,args.db)
          except sqlite3.Error as e:
            raise exceptions.db_error(e)
        print(reading.as_csv(unit))
      if args.server is not None:
        n=0 #counter
        while (len(readings)>n):
          sync.sync_data(args.server, args.db)
          n += 1
      else:
        raise exceptions.server_error('Server not present.')
      status.show_status(2,"Sync successful!")
